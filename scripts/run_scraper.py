...
from config.settings import RAW_JSON_PATH, RAW_DIR, PROCESSED_DIR, INSIGHTS_DIR, SCREENSHOTS_DIR, OUTPUT_DIR, GEM_BASE_URL, DEFAULT_TIMEOUT
...
async def run_pipeline() -> None:
    ...
    async with BrowserManager() as browser_manager:
        ...
        print('─' * 60)
        bids = await scrape_all_listings(page)
        if not bids:
